...
class SpeakerMatcher:
    ...
    def _extract_identity_words(self, text: str) -> List[str]:
        ...
        来源：基于错误案例分析（P1、P21、P29、P34 等身份词未被识别案例）
    ...
    def _extract_context_speakers(
        ...
        # 步骤3：身份词提取（补充 NER 未覆盖的身份/职业词场景）
        # 用途：识别"掌柜"、"首领"、"联盟成员"等身份词作为说话人候选
        # 来源：基于错误案例分析（P1、P21、P29、P34 等身份词未被识别案例）
        # 边界：仅匹配 2-4 字的身份词，避免匹配长描述
        # 更新日期：2026-05-10
        # 维护者：P0 身份词修复方案
            ...
            if context_text:
                ...
                for word in identity_words:
                    if word not in seen_names:
                        seen_names.add(word)
                        char = self._match_candidate_to_character(word, context_text, skip_speech_check=True)
                        if char:
                            conf = 0.70 if char.id != -1 else 0.55
                            source = 'identity_word(前)' if context_text == context_before else 'identity_word(后)'
                            candidates.append((char.name, source, conf))
                        else:
                            source = 'identity_word(前)' if context_text == context_before else 'identity_word(后)'
                            candidates.append((word, source, 0.50))
    ...
    def match_speaker(self, context: DialogueContext) -> Optional[MatchResult]:
        if context.context_before or context.context_after:
            ...
            if context_candidates:
                ...
                # _extract_context_speakers 已经通过 _match_candidate_to_character 匹配到角色库
                # 直接查找角色（此时应该已在角色库中）
