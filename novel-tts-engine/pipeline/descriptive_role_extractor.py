...
# TITLE_TRIGGERS
#
# 用途：头衔触发词，用于识别"XX头衔"格式的角色指称
# 来源：中文社会称谓体系——职衔类（约25条）
# 边界：仅包含常见中文头衔，不应无限制扩容
# 更新日期：2026-05-02
# 维护者：项目规则
TITLE_TRIGGERS = [
...
# ACTION_TRIGGERS
#
# 用途：动作触发词，用于识别"XX做了某事"从而推断XX是说话人
# 来源：中文网文常见动作描写词汇（约25条）
# 边界：仅包含能暗示说话人身份的动作词，不应往里加通用动作
# 更新日期：2026-05-02
# 维护者：项目规则
ACTION_TRIGGERS = [
...
# ADDRESS_TRIGGERS
#
# 用途：称呼触发词，用于识别"XX，..."格式的直接称呼
# 来源：中文社会称谓体系——亲属/社交称呼类（约15条）
# 边界：仅包含常见中文社交/亲属称呼，用于称呼匹配场景
# 更新日期：2026-05-02
# 维护者：项目规则
ADDRESS_TRIGGERS = [
    '兄台', '兄', '弟', '妹', '姐', '叔', '伯', '婶', '姨',
