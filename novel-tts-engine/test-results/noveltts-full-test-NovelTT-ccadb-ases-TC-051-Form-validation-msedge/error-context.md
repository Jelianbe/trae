...
- Name: noveltts-full-test.spec.js >> NovelTTS Cloud Full Test >> Module 6: Error Handling & Edge Cases >> TC-051: Form validation
- Location: tests\e2e\noveltts-full-test.spec.js:798:5
...
> 803 |       await page.click('[data-action="open-modal"][data-modal="new-project"]');
  ...
  882 |       ];
  883 | 
  884 |       let invisibleCount = 0;
  885 |       for (const selector of toolbarButtons) {
  886 |         const btn = page.locator(selector);
  887 |         const isVisible = await btn.isVisible().catch(() => false);
  888 |         if (!isVisible) {
  889 |           console.log('  [WARN] Button not visible:', selector);
  890 |           invisibleCount++;
  891 |         }
  892 |       }
  893 | 
  894 |       console.log('  Invisible buttons:', invisibleCount);
  895 |       console.log('  [PASS] Toolbar buttons checked');
  896 |     });
  897 |   });
  898 | 
  899 |   test.afterAll(async () => {
  900 |     console.log('\n========== Test End ==========');
  901 |     console.log('API requests:', apiRequests.length);
  902 |     console.log('Console errors:', consoleErrors.length);
  903 |   });
```