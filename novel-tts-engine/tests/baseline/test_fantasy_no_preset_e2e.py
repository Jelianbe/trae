"""端到端说话人识别准确率测试 — 西幻-无预注册

标准化基线测试 #4（E2E）：完整文本导入 → 自动分割 → 说话人识别
...
用法: python tests/baseline/test_fantasy_no_preset_e2e.py
"""
import sys
import re
import json
...
def run():
    ...
    try:
        ...
        matcher._current_project_id = 'fantasy_no_preset_e2e'
...
if __name__ == "__main__":
    ...
    print(f"西幻-无预注册 (E2E): {accuracy:.1f}% ({correct}/{judgable}) [{status}] (预期≥{EXPECTED_BASELINE}%)")
