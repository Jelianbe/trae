...
用法: python tests/baseline/test_fantasy_preset.py
"""
import sys
import re
import json
...
def run():
    ...
    try:
        ...
        for name, info in CHARACTER_INFO.items():
            char_manager.add_character(name, project_id='fantasy_baseline', aliases=info['aliases'], gender=info['gender'])
        ...
        for i, d in enumerate(dialogues):
            ...
            if result and result.character:
                ...
                predicted, match_type, confidence = 'UNKNOWN', 'none', 0.0
            if expected == 'UNKNOWN':
...
if __name__ == "__main__":
    ...
    print(f"西幻-有预注册: {accuracy:.1f}% ({correct}/{total}) [{status}] (预期≥{EXPECTED_BASELINE}%)")
    for r in results:
        if r['status'] == 'wrong':
