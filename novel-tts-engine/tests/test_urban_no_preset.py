...
用于验证 H-20260516-10 频次过滤机制在动态角色发现场景下的效果。
不预注册任何角色，让系统在分析文本时动态发现角色。

运行方式: python tests/test_urban_no_preset.py
"""
...
def main():
    ...
    result_path = project_root / 'tests' / 'urban_no_preset_result.json'
    ...
    matcher._current_project_id = 'test_no_preset'
    ...
    for name in sorted(created_chars):
        freq = char_manager.get_frequency(name, 'test_no_preset') if hasattr(char_manager, 'get_frequency') else 'N/A'
